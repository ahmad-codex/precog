...
describe("Test Precog 6", async () => {
    ...
    describe("Trading cycle 0", async () => {
        it("User1 deposit 100000 USDC", async () => {
            await precogV5.connect(clients[0]).deposit(usdc.address, "100000000000");
        });

        it("User2 deposit 100000 USDC", async () => {
            await precogV5.connect(clients[1]).deposit(usdc.address, "100000000000");
        });
        ...
        it("Get current trading cycle", async () => {
            const blockNumBefore = await ethers.provider.getBlockNumber();
            const blockBefore = await ethers.provider.getBlock(blockNumBefore);
            const timestampBefore = blockBefore.timestamp;
            console.log(timestampBefore);

            const tradingCycle = await precogInternal.getCurrentTradingCycle(usdc.address);
            console.log(`Id: ${tradingCycle.id} | StartTime: ${tradingCycle.startTime} | EndTime: ${tradingCycle.endTime}`)
        });

        it("User1 request withdrawal first", async () => {
            await network.provider.send("evm_increaseTime", [300]);
            await precogV5
                .connect(clients[0])
                .requestWithdrawal(usdc.address, "99900000000");
        });

        it("User3 request withdrawal first", async () => {
            await precogV5
                .connect(clients[2])
                .requestWithdrawal(usdc.address, "99900000000");
        });

        it("User1 withdraw from register", async () => {
            await network.provider.send("evm_increaseTime", [600]);
            await precogV5
                .connect(clients[0])
                .withdraw(clients[0].address, usdc.address, "99900000000", false);
        });

        it("User3 withdraw from register", async () => {
            await precogV5
                .connect(clients[2])
                .withdraw(clients[2].address, usdc.address, "99900000000", false);
        });
        
        it("Set configuration of cycles", async () => {
            //await network.provider.send("evm_increaseTime", [300]);
            await precogCore.setCycleConfiguration([14400, 300, 300, 14400]);
        ...
        it("User2 request withdrawal first", async () => {
            await network.provider.send("evm_increaseTime", [300]);
            await precogV5
                .connect(clients[1])
                .requestWithdrawal(usdc.address, "99900000000");
        ...
        it("User1 request withdrawal", async () => {
            await network.provider.send("evm_increaseTime", [300]);
            await precogV5
                .connect(clients[0])
                .requestWithdrawal(usdc.address, "99900000000");
        ...
        it("Check investment of", async () => {
            ...
            console.log(investmentOfUser1);
            console.log(investmentOfUser2)
        });
