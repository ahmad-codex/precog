...
describe("Test Precog 4", async () => {
    ...
    describe("Trading cycle 2", async () => {
        ...
        it("User4 request withdrawal 1000 USDC", async () => {
            await network.provider.send("evm_increaseTime", [1]);
            await precogV5
                .connect(clients[3])
                .requestWithdrawal(usdc.address, "1000000000");
        });
    ...
    describe("Trading cycle 3", async () => {
        ...
        it("User4 withdraw 3334.335618 USDC from register", async () => {
            await network.provider.send("evm_increaseTime", [136]);
            await precogV5
                .connect(clients[3])
                .withdraw(clients[3].address, usdc.address, "3334335618", false);
        });
    ...
    describe("SendProfit", async () => {
        ...
        it("Sendprofit for cycle 4", async () => {
            await network.provider.send("evm_increaseTime", [1000]);
            await precogProfit.sendProfit(usdc.address, "1000000000");
        });

        it("Sendprofit for cycle 5", async () => {
            await network.provider.send("evm_increaseTime", [1000]);
            await precogProfit.sendProfit(usdc.address, "1000000000");
        });
    });
    ...
    describe("takeProfit", async () => {
        it("takeProfit", async () => {
            ...
            const balancePCOG = await pcog.balanceOf(precogV5.address);
            const balanceUSDC = await usdc.balanceOf(precogV5.address);
