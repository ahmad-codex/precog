...
describe("Test Precog 1", async () => {
    ...
    describe("Trading cycle 0", async () => {
        it("User1 deposit 1000 USDC", async () => {
            const txs = await precogV5.connect(clients[0]).deposit(usdc.address, "1000000000");
        ...
        it("User1 transfer 499 IPCOG for user2", async () => {
            await network.provider.send("evm_increaseTime", [163]);
            const liquidityTokenAddress = await precogStorage.getTokenConvert(
                usdc.address
            );
            const IPCOG = await ethers.getContractFactory("IPCOG");
            ...
            await liquidityToken
                .connect(clients[0])
                .transfer(clients[1].address, "499000000")
                .catch((err) => { });
        });
        ...
        it("User2 transfer 99.9 IPCOG for user3", async () => {
            ...
            await liquidityToken
                .connect(clients[1])
                .transfer(clients[2].address, "99900000")
                .catch((err) => { });
        });
    });
    ...
    describe("Trading cycle 1", async () => {
        ...
        it("Admin set user3 into whitelist", async () => {
        ...
        it("User1 request withdrawal all", async () => {
            ...
            const txs = await precogV5
                .connect(clients[0])
                .requestWithdrawal(usdc.address, "500000000");

                const receipt = await txs.wait() 
                console.log(receipt.gasUsed);
        });

        it("User1 deposit 1500 USDC", async () => {
            await network.provider.send("evm_increaseTime", [56]);
            await precogV5.connect(clients[0]).deposit(usdc.address, "1500000000");
        });
    });
    ...
    describe("Trading cycle 2", async () => {
        ...
        it("User1 request withdrawal all", async () => {
            ...
            await precogV5
                .connect(clients[0])
                .requestWithdrawal(usdc.address, "1997500000");
        });
    });
    ...
    describe("Trading cycle 4", async () => {
        ...
        it("User1 withdraw 1498.5 USDC", async () => {
            await precogV5
                .connect(clients[0])
                .withdraw(clients[0].address, usdc.address, "1997500000", false);
        });
        ...
        it("User3 withdraw 599.4 USDC", async () => {
            await precogV5
                .connect(clients[2])
        ...
        it("Check calculateProfit of users", async () => {
            const totalProfitForPCOG = await pcog.balanceOf(precogProfit.address);
            const totalProfitForToken = await usdc.balanceOf(precogProfit.address);
            const profitOfUser1 = await precogInternal.calculateProfit(usdc.address, clients[0].address);
            const profitOfUser2 = await precogInternal.calculateProfit(usdc.address, clients[1].address);
            const profitOfUser3 = await precogInternal.calculateProfit(usdc.address, clients[2].address);

            console.log("totalProfitForPCOG:", totalProfitForPCOG.toString());
            console.log("totalProfitForToken:", totalProfitForToken.toString());
            console.log("profitOfUser1ByPcog:", (profitOfUser1.profit).toString());
            console.log("profitOfUser2ByPcog:", (profitOfUser2.profit).toString());
            console.log("profitOfUser3ByPcog:", (profitOfUser3.profit).toString());
            console.log("profitOfUser1ByToken:", (profitOfUser1.profitForWhitelist).toString());
            console.log("profitOfUser2ByWhitelist:", (profitOfUser2.profitForWhitelist).toString());
            console.log("profitOfUser3ByWhitelist:", (profitOfUser3.profitForWhitelist).toString());
